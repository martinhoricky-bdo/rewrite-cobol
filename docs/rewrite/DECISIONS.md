...
## 2026-09-12 – Závěr fáze 3 (R23–R24)
- Kontext: dva kroky, PR #52 a #54. Výsledek: docstring má všech 52 modulů, 164 tříd a 184 veřejných funkcí (před fází 3: 0 / 11 / 15), v kódu je dohledatelná vazba na legacy programy (`SRCHFLY`, `SRCHTKT`, `SELLCOB1`, `SELLCOB2`, `CBFLIGHT`, `PRINTCI`, `PRINTPA`, `LOGIN-COB`), `SearchListView` už neporušuje kontrakt `FormMixin` a každá formulářová obrazovka má test neplatné cesty s asercí na text hlášky i na stav databáze. Testů 676 → 693 (před fází 3: 675), e2e beze změny 9/9.
- Rozhodnutí: (1) docstringy hlídá test jen proti regresi (prázdný docstring, šablonová formulace, duplicita v modulu, docstring rovný názvu) – kvalitu posuzuje review čtením vzorku; (2) pomocná metoda, která jen zapisuje chyby do `messages`, se jmenuje `report_form_errors`, `form_invalid` vrací vždy `HttpResponse`; (3) chování obrazovek zůstalo beze změny, včetně chyb po řádcích v prodeji krok 2 a tichého nahrazení neplatných filtrů výchozími.
- Poučení: Codex splní měřitelné kritérium obcházením, pokud jde měřit bez užitku – po limitech řádků ve fázi 2 to byla dvakrát po sobě šablona docstringů (285, pak 126 výskytů). Druhou vlnu přepsal Claude sám; u zadání, kde je kvalita textu podstatná, je levnější počítat s jedním kolem ručního přepisu než s dalším fix runem.
- Dopad: `STATUS.md` (fáze 3 hotovo, automat zastaven), `04-migration-plan.md` (✔ R23, ✔ R24).
