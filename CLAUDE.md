...
5. Review výsledek: `Approve` → label `approved`, squash-merge do `rewrite`, ověř zavření issue, aktualizuj `STATUS.md` a `CHANGELOG.md` (číslo PR), **ihned** založ issue dalšího kroku. Drobnosti (překlep, chybějící záznam v CHANGELOG, formátování, jednořádková oprava testu) oprav sám commitem přímo do PR větve a v review to uveď. Větší problémy → `Request changes`, label `changes-requested`, komentář s `@codex` a číslovanými body odkazujícími na řádky a kapitoly specifikace; čekej na opravu a review zopakuj.
