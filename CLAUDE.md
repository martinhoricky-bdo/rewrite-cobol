...
3. Přidej komentář se zmínkou (příkaz pushe je povinný v každém zadání i fix runu – push přes `origin` u Codexu selhává na 403, viz `AGENTS.md` kap. 4a): `@codex Implementuj krok R<ID> podle zadání v tomto issue. Nejdřív ověř `gh auth status`. Pushni větev NE přes origin, ale přímo: `git push "https://x-access-token:$(gh auth token)@github.com/martinhoricky-bdo/rewrite-cobol.git" HEAD:refs/heads/codex/R<ID>-<slug>`, pak otevři PR do větve `rewrite` (ne `main`) přes `gh pr create --base rewrite --head codex/R<ID>-<slug> …`, v popisu uveď `Closes #<issue>`. Pokud `gh auth token` nebo push selže, napiš sem kompletní výstup `gh auth status` a přesnou chybu z `git push`.` Zmínka spouští Codex Cloud task (GitHub Mention).
