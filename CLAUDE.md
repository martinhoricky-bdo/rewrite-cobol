...
3. Přidej komentář se zmínkou: `@codex Implementuj krok R<ID> podle zadání v tomto issue. Otevři PR do větve `rewrite` (ne `main`) z větve `codex/R<ID>-<slug>`, v popisu uveď `Closes #<issue>`.` Zmínka spouští Codex Cloud task (GitHub Mention).
4. Naplánuj kontrolu (send_later ~15 min, poté hodinově): jakmile PR existuje, `subscribe_pr_activity`, nastav label `needs-review`, proveď review podle checklistu níže.
5. Review výsledek: `Approve` → label `approved`, squash-merge do `rewrite`, smaž větev, ověř zavření issue, zapiš stav do `CHANGELOG.md` (číslo PR), založ issue dalšího kroku. `Request changes` → label `changes-requested`, body s číslovanými body odkazujícími na řádky a kapitoly specifikace; po opravě Codexem znovu review.
6. Uživateli pošli krátké shrnutí po každém merge nebo blokujícím problému; jinak nepiš.
...
Přebírání PR od Codexu: nikdy do jeho větve nepushuj opravy sám – výhrady jdou přes review; jen konflikt s `rewrite` můžeš vyřešit merge commitem (bez rebase).
