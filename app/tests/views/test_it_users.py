...
def test_reset_password_requires_change_after_login(client):
    login_as(client, 6)
...
def test_user_list_no_password_status_and_pagination(client):
    login_as(client, 6)
...
def test_hr_cannot_view_users(client):
    login_as(client, 5)
    assert client.get(reverse("it:users")).status_code == 403
...
def test_sales_cannot_post_user_actions(client, url_name):
    login_as(client, 7)
